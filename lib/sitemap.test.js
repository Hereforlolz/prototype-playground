...
test('sitemap includes every static page, case-study page, and experiment page exactly once', () => {
  const paths = buildSitemapPaths();
  const expectedCount = 4 + Object.keys(CASE_STUDIES).length + Object.keys(EXPERIMENTS).length;
...
test('every experiment slug is present as an /experiments/<slug> path', () => {
  const paths = buildSitemapPaths();
  for (const slug of Object.keys(EXPERIMENTS)) {
    assert.ok(paths.includes(`/experiments/${slug}`), `missing sitemap entry for ${slug}`);
  }
});
