...
test('Therapist Dashboard is explicit about the missing authentication and synthetic-data scope', () => {
  ...
  // pilot line drifting into a measured-result claim.
  const study = CASE_STUDIES.therapistDashboard;
  const scopeText = study.scope.toLowerCase();
  const whatBrokeText = study.whatBroke.join(' ').toLowerCase();

  assert.match(scopeText, /synthetic patient data/);
  assert.match(scopeText, /not hipaa-compliant/);
  assert.match(whatBrokeText, /no authentication/);
  // The "~40%" line must stay attributed to the pilot therapist's own
  // characterization, not asserted as a measured outcome of the tool.
  const outcomeText = study.outcome.join(' ');
