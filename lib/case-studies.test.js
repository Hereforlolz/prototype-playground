...
const EXPECTED_SLUGS = ['teamtrail', 'qwenMemoryAgent', 'deadCodeFinder', 'compassionateConnect', 'greenGrid'];
...
test('CompassionateConnect is explicit that it is not a validated clinical tool', () => {
  ...
  const allText = [study.scope, ...study.outcome].join(' ').toLowerCase();

  assert.match(study.scope.toLowerCase(), /archived.*demo only|demo only.*archived/);
  assert.match(study.scope.toLowerCase(), /no real phi|simulated patient data/);
  assert.match(allText, /not.*(validated|clinical)|no eval harness/);
});
