...
test('splitHighlightedProjects separates known-private entries into inProgress', () => {
  const highlighted = buildHighlightedProjects([]);
  const { available, inProgress } = splitHighlightedProjects(highlighted);

  assert.equal(available.length + inProgress.length, highlighted.length);
  assert.ok(available.every(h => Boolean(h.url)));
