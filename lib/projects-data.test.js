...
test('exactly the flagship projects are featured: TeamTrail, Qwen MemoryAgent, Dead Code Finder', () => {
  const highlighted = buildHighlightedProjects([]);
  const { flagship } = splitHighlightedProjects(highlighted);

  assert.deepEqual(
    flagship.map(h => h.name).sort(),
    ['Hereforlolz/qwen-memory-agent', 'Hereforlolz/teamtrail', 'gitlab/gitlab-ai-hackathon/transcend']
      .sort()
  );
