...
test('CompassionateConnect\'s blurb does not overclaim the FunctionTool report and cites the correct repo', () => {
  // Regression guard: this blurb used to say "a reproducible bug" (an
  // unconfirmed report stated as settled fact) filed against
  // "adk-python" (the wrong repo — the actual issue, #826, lives in
  // adk-docs). Both were corrected; this locks the correction in place.
  const compassionateConnect = HIGHLIGHTS.find(h => h.name === 'Hereforlolz/Compassionate-connect');
  const blurb = compassionateConnect.blurb.toLowerCase();

  assert.equal(blurb.includes('reproducible bug'), false);
  assert.equal(blurb.includes('adk-python'), false);
  assert.ok(blurb.includes('adk documentation repository') || blurb.includes('adk-docs'));
});
