...
// Ordered by how differentiated the work is, not by recency: the two
// hackathon-judged, technically deeper projects lead, then the smaller
// single-purpose tools, then the private-repo entries (see note above)
// last, since those get demoted to a separate "in progress" group by
// splitHighlightedProjects() regardless of their position here.
const HIGHLIGHTS = [
  {
...
// Splits curated highlights into ones a visitor can actually open (a real
// url — public repo or case study) and ones still in progress (no url,
// currently the known-private repos). Keeps "flagship" project cards free
// of unfinished/inaccessible entries without hiding them from the page
// entirely — they render in their own, clearly-labeled secondary group.
function splitHighlightedProjects(highlighted) {
  return {
    available: highlighted.filter(h => h.url),
