...
test('no bug-log entry asserts an unconfirmed report is a confirmed bug', () => {
  // Regression guard: adk-docs#826 is still open and unconfirmed by
  // Google — a maintainer ruling on the closely related (and not hers)
  // #1331 concluded that kind of FunctionTool TypeError was expected API
  // behavior, not a defect. "why" text for any entry must not overclaim
  // resolution/severity as settled fact ahead of maintainer confirmation.
  const overclaimPhrases = ['reproducible bug', 'severe runtime bug', 'confirmed bug', 'confirmed runtime bug'];
  for (const bug of bugs) {
    const text = bug.why.toLowerCase();
    for (const phrase of overclaimPhrases) {
      assert.equal(text.includes(phrase), false, `"${bug.title}" why-text should not include "${phrase}"`);
    }
  }
});
