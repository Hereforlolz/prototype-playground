...
const CASE_STUDIES = {
  ...
  // page; it's presented here as the pilot therapist's own
  // characterization of her documentation burden, not as a measured
  // result of using this specific tool. The lack of authentication is
  // confirmed directly in template.yaml (no authorizer/Cognito/API key
  // configured) and in every Lambda handler (no token check at all);
  // the account owner has confirmed the pilot used synthetic patient
  // data only, precisely because auth wasn't yet in place.
  therapistDashboard: {
    slug: 'therapist-dashboard',
    title: 'AI-Powered Therapist Dashboard',
    subtitle: 'A serverless dashboard that turns messy session notes into summaries and supportive, non-diagnostic insights',
    tagline: 'Built solo for the AWS Lambda Hackathon, June 2025 — an early lesson in shipping auth from day one',
    ...
    problem: [
      'Therapists lose a meaningful share of their time to session documentation instead of patient care — the project’s pilot user, a practicing therapist, confirmed this matched her real experience.',
      'Turning messy, informal session notes into a usable summary and gentle follow-up direction is exactly the kind of repetitive synthesis work an LLM can help with — if it’s built carefully around a sensitive domain.',
    ],
    approach: [
      'Built as a serverless pipeline: a React frontend calls three separate Lambda functions behind API Gateway — one to list patient summaries, one to save or update a session summary, and one that sends session notes to Claude (via Bedrock) and returns insights.',
      'The Bedrock prompt is deliberately scoped: it explicitly frames Claude’s role as generating "gentle," "supportive," and "professional" insights about patient progress and next steps, not diagnoses.',
      'Data lives in DynamoDB, with the architecture designed so PHI isn’t held in client-side browser state — described as "HIPAA-aligned," with full compliance work named explicitly as not yet done.',
    ],
    whatBroke: [
      'No authentication was implemented on any of the three Lambda endpoints — confirmed directly in the code and in template.yaml, which has no Cognito authorizer, API key, or request validation configured at all. Anyone with the API Gateway URL could have read or written patient records. The pilot used synthetic patient data throughout for exactly this reason — a real lesson from a first hackathon, not a deliberate tradeoff.',
    ...
    outcome: [
      'A real practicing therapist piloted the dashboard directly and confirmed that session documentation was consuming a substantial share of her time — validating the problem the dashboard targets — and said she’d use a version of it.',
      'The full pipeline works end to end on serverless infrastructure with no servers to manage: notes in, a Claude-generated insight and a saved, retrievable summary out.',
    ],
    scope:
      'A first-hackathon build (AWS Lambda Hackathon, June 2025), piloted using synthetic patient data only — the account owner has confirmed no real patient records were ever entered, precisely because authentication wasn’t in place yet. The dashboard is HIPAA-aligned in its data architecture but is explicitly not HIPAA-compliant, and its own README lists real authentication, role-based access, and full compliance as future work rather than something already built.',
  },
};
