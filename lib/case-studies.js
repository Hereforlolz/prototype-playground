// lib/case-studies.js
//
// Full case-study content for eight projects with a dedicated
// /projects/<slug> page: the three flagship-tier builds (TeamTrail, Qwen
// MemoryAgent, Dead Code Finder), four case-study-tier builds
...
// for which group each renders in on /projects — plus one workplace
// retrospective (Prompt Engineering Adoption at Ivani) that's
// deliberately excluded from HIGHLIGHTS/projects-data.js and only
// discoverable via internal links from /about and /projects. Kept as
// plain data — no JSX — so it's testable with Node's built-in test
// runner and reusable if the case study pages ever need to share
// layout. Every claim in the first seven case studies is drawn from the
// projects' own repo READMEs (architecture, "Known gotchas"/"Known
// limitations"/"What I'd Do Differently" sections, test suites) — no
// invented user counts, adoption numbers, or benchmarks. The
// promptEngineeringAdoption entry below is sourced differently — an
// owner-authored retrospective plus dated personal notes, not a repo
// README — see the sourcing comment above that entry.
'use strict';
...
const CASE_STUDIES = {
  ...
  qwenMemoryAgent: {
    ...
    // not a mockup — but not a live backend run either. Precisely, so
    // nothing here overstates what's shown:
    // - The user-message scenario (the deadline exchange) is sourced from
    //   existing project test/demo material, not invented: the pre-existing
    //   memory cards are seed_memories.py's own seed facts verbatim
    //   (SEED_USERNAME "nidhi"), and the deadline exchange is the exact user
    //   messages from test_memory_agent.py's test_cross_session_recall.
    // - The frontend is the real application, unmodified.
    // - The backend responses were intercepted: no Qwen API key exists in
    //   this build environment, so /chat and /chat/memories were mocked in
    //   a real browser session rather than calling a live model.
    // - The importance scores and expiry dates are synthetic example values
    //   consistent with the documented behavior (high importance ->
    //   permanent), not captured output.
    // - No live Qwen scoring or recall call occurred for these images.
    // Only two slots are used (no hero) — this project doesn't need a third
    // image.
    media: {
      afterApproach: {
        ...
        width: 2400,
        height: 1520,
        alt: 'Qwen MemoryAgent demo showing a stored project-deadline memory in the sidebar with an example importance score and permanent expiry.',
        label: 'What this shows',
        caption: 'In this synthetic demo flow, the UI makes the memory decision visible: a fact can be marked as stored, assigned an importance score, and given an expiry instead of disappearing into ordinary chat history.',
      },
      afterOutcome: {
        src: '/case-studies/qwen-memoryagent/memory-recalled.png',
        width: 1760,
        height: 492,
        alt: 'Qwen MemoryAgent demo of a second, synthetic session: the user asks about the deadline again and the interface surfaces the earlier memory in its response.',
        label: 'What this demonstrates',
        caption: 'A second synthetic session shows the intended recall experience: the user asks about the deadline again and the interface surfaces the earlier memory in the response. The repo’s tests cover the underlying cross-session recall behavior.',
      },
    },
    stack: [
  ...
  // Corrected after re-verifying the FunctionTool bug citation against the
  // public GitHub record: the whatBroke text below previously cited
  // google/adk-python#1331, an issue filed anonymously by someone else that
  // happens to describe a similar-sounding TypeError. The issue actually
  // authored by this site's owner is google/adk-docs#826 (matches
  // bugs.json's existing, already-correct link) — the citation and the
  // surrounding technical description were both corrected to match #826's
  // real content instead of #1331's.
  compassionateConnect: {
    ...
    explanations: {
      simple: {
        sections: [
          ...
          {
            title: 'Why is that useful?',
            text: 'Splitting the work into agents with clear boundaries makes it possible to give safety rules — like flagging a crisis or refusing to diagnose — to one specific agent as its explicit, narrowly scoped job, rather than hoping a single system behaves correctly across every kind of conversation. That’s the intended design boundary, not a tested guarantee it can’t be circumvented — see below.',
          },
          {
            title: 'What is still unproven?',
    ...
    whatBroke: [
      'Refactoring toward Google’s Agent Development Kit hit a persistent TypeError initializing FunctionTool on google-adk==1.2.1. The issue report documented argument-parsing behavior involving functions with mixed required and optional parameters, reproduced on Python 3.13 and 3.11. After recording the environment and diagnostic evidence, I filed google/adk-docs issue #826 and continued development with a workaround. This is the same issue tracked in this site’s Manual Bugs Log.',
  ...
  pilotCraft: {
    ...
    media: {
      ...
      afterOutcome: {
        ...
        label: 'What this shows',
        caption: 'A different synthetic scenario — reviewing legal contracts for liability — where the tool rated AI a poor fit and said so, down to a stop condition against letting it make binding decisions at all. Fixture data, not a live model call — see note above.',
      },
    },
    stack: [
  ...
  // Sourced from an owner-authored retrospective (written September 2026)
  // describing a self-run experiment at Ivani, based on personal working
  // notes dated April-May 2026 — not a repository README, and not an
  // internal Ivani document. No public repo, Devpost entry, or source
  // document exists for this case study; repoUrl/devpostUrl are
  // intentionally omitted rather than invented (see hasAnyCaseStudyLink
  // in lib/case-study-links.js, which components/CaseStudy.js uses to
  // skip the action-button row entirely when none exist). The source
  // material itself (a private document) is never linked or reproduced
  // here — only this synthesized, safety-reviewed summary.
  //
  // Framing was deliberately softened during review: "rolled out tools
  // with no guidance" became "engineers were experimenting without a
  // shared usage guide," "zero-cost" became "no external API cost," and
  // every quantitative claim (the ~50% rework / ~8 minute figures, the
  // ~10-person/two-week adoption spread) is labeled a directional,
  // self-observed estimate — never a measured, surveyed, or
  // independently validated result. Local resource observations (CPU/
  // memory via Task Manager) are kept explicitly separate from the
  // qualitative comparison of model responses; Task Manager showed
  // resource usage, not model reasoning, context usage, or output
  // quality. No token-count savings are claimed, since none were
  // recorded. No confidential prompts, colleague identities, proprietary
  // code, or internal screenshots are reproduced.
  promptEngineeringAdoption: {
    slug: 'prompt-engineering-adoption',
    title: 'Prompt Engineering Adoption at Ivani',
    subtitle: 'A small-team AI adoption rollout built from observed workflow friction',
    tagline: 'Retrospective written in September 2026 from personal working notes dated April–May 2026',
    stack: [
      ['Setting', 'An approximately 10-person IoT company (Ivani)'],
      ['Local test environment', 'Ollama running Qwen, on a local machine'],
      ['Test material', 'Sanitized firmware-team queries involving the ESP-Zigbee SDK'],
      ['Comparison', 'Existing, as-written engineer prompts versus deliberately structured versions of the same queries'],
      ['Deliverables', 'A prompt template, a cheat sheet, context-window guidance, and a system-prompt template'],
    ],
    problem: [
      'As AI tools became available across Ivani, engineers were experimenting without a shared usage guide — prompting styles varied widely, and nobody had checked whether the differences actually changed the answers.',
      "Firmware-team queries involving the ESP-Zigbee SDK were a natural test case: technical enough to expose real prompting friction, and specific enough that a vague prompt's failure mode — context drift, a confidently invented API detail — would be easy to spot.",
    ],
    approach: [
      'Ran existing, as-written engineer prompts and deliberately restructured versions of the same queries side by side, locally, using Ollama running Qwen against sanitized firmware-team queries about the ESP-Zigbee SDK, at no external API cost.',
      'Local resource observations (CPU and memory on the test machine, via Task Manager) were kept separate from the qualitative comparison of the responses themselves — accuracy, whether the model stayed on the original constraint across a multi-turn exchange, and how much follow-up clarification each version needed. Task Manager showed local resource usage, not model reasoning, context usage, or output quality.',
      'The structured prompts more consistently held onto the original constraint across turns and asked for clarification instead of guessing when a query was ambiguous. The existing, as-written prompts more often produced a plausible-sounding but unverified API detail with no hedge attached.',
      'Turned the comparison into reusable material rather than a one-off writeup: a prompt template, a one-page cheat sheet, context-window guidance (how much surrounding code or spec to include before responses degraded), and a system-prompt template other engineers could adapt directly.',
    ],
    whatBroke: [
      'Existing, as-written prompts frequently let the model answer with unwarranted confidence — a wrong SDK function signature stated as fact, with no signal that the model was guessing. Prompts that explicitly asked the model to flag uncertainty reduced this, though not to zero.',
      'Longer, unstructured exchanges were where context drift showed up most: several turns in, the model would lose track of a constraint stated earlier, like a specific SDK version. Prompts that restated key constraints held up noticeably better across turns.',
      'There was no tooling to measure any of this precisely. This was a self-run, informal comparison, not an instrumented experiment — "which response was better" was a qualitative judgment call, not a scored metric.',
    ],
    outcome: [
      'Adoption spread by demonstrated usefulness rather than a formal training rollout: two engineers began using the templates first, and use spread across the roughly 10-person team over about two weeks.',
      'Observed estimates from this self-run comparison — not independently validated research — put the effect at a directional estimate of approximately 50% less engineer rework and a directional estimate of approximately 8 minutes saved per engineering query. There was no formal survey, no usage dashboard, no time-to-proficiency measurement, and no controlled adoption study behind either number.',
      "The reusable output — the prompt template, cheat sheet, context guidance, and system-prompt template — is what actually carried the finding into the team's daily use, not the comparison itself.",
    ],
    scope:
      "A retrospective written in September 2026 from personal working notes dated April–May 2026, not an internal Ivani document. Reflects one person's self-run, informal comparison at no external API cost, on a single local machine, using sanitized firmware-team queries — not a controlled study, and not independently validated. No confidential prompts, colleague identities, proprietary code, or internal screenshots are reproduced here.",
  },
};
