// lib/case-studies.js
//
// Full case-study content for all six projects with a dedicated
// /projects/<slug> page: the three flagship-tier builds (TeamTrail, Qwen
// MemoryAgent, Dead Code Finder) plus three case-study-tier builds
// (CompassionateConnect AI, GreenGrid AI, AI-Powered Therapist
// Dashboard) — see the `tier` values in lib/projects-data.js for which
// group each renders in on /projects. Kept as plain data — no JSX — so
// it's testable with Node's built-in test runner and reusable if the
// case study pages ever need to share layout. Every claim here is drawn
// from the projects' own repo READMEs (architecture, "Known
