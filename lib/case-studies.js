...
const CASE_STUDIES = {
  ...
  pilotCraft: {
    ...
    // Plain-language explanation for a smart-novice reader. Translates the
    // Overview below rather than shortening it, and deliberately stays out
    // of architecture/implementation territory — that material lives in
    // the repo linked above, not duplicated here. Every claim traces back
    // to the problem/approach/outcome/scope fields below, which are
    // themselves sourced from the project's own README.
    explanations: {
      simple: {
        sections: [
          {
            title: 'What was happening',
            text: 'Inside a lot of workplaces, people had already started using AI tools on their own — pasting text into chatbots, trying AI features bolted onto existing software — before anyone agreed on the rules. Teams considering an official AI project would often jump straight to "let’s build an AI feature" without stopping to separate what they actually knew from what they were just assuming, and without deciding upfront what would even count as success.',
          },
          {
            title: 'Why it mattered',
            text: 'Building an AI feature takes real engineering time, and once it exists, it’s hard to quietly walk back. Most of these decisions were being made on a gut feeling instead of any structured check. Nobody had a first pass that was actually allowed to say "no, this isn’t a good fit" before anyone committed real time to it.',
          },
          {
            title: 'What I made',
            text: 'PilotCraft takes a plain-English description of a work problem and turns it into one structured plan: whether AI is a strong, conditional, or poor fit; a step-by-step trial with who owns what and what evidence to collect at each step; how you’d know it’s working, with a real starting point instead of an invented target; and exactly what would make you stop, revise, or scale it up. It’s built to genuinely consider a non-AI fix first, and to say plainly when a claim isn’t backed by evidence instead of inventing a number to fill the gap.',
          },
          {
            title: 'What happened',
            text: 'A live run against the real AI model, across nine made-up example scenarios covering both strong and weak candidates for AI, passed all 58 automated checks built to catch things like an invented baseline or a skipped evidence gap. A separate set of 74 tests confirms the underlying machinery holds up under stress — retrying if the AI call fails, catching malformed output, rejecting oversized requests. It’s still a personal tool, though: no accounts, no live deployment, and no real organization has run an actual decision through it yet.',
          },
        ],
      },
    },
